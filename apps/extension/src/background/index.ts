...
const SELECTOR_CONFIG_STORAGE_KEY = 'syncsellr_selector_config';
const SELECTOR_HEALTH_STORAGE_KEY = 'syncsellr_selector_health';
const API_BASE = 'http://localhost:4000';

// ─── Selector Config Refresh ────────────────────────────────────────────────

async function refreshSelectorConfig(): Promise<void> {
  console.log('[SyncSellr BG] Refreshing selector config from API...');
  try {
    ...
    if (response.status === 304) {
      console.log('[SyncSellr BG] Selector config is up to date');
      return;
    }

    if (!response.ok) {
      console.warn(`[SyncSellr BG] Selector config fetch failed: HTTP ${response.status}`);
      return;
    }
    ...
    if (config?.version && config?.platforms) {
      await new Promise<void>((resolve) => {
        chrome.storage.local.set({ [SELECTOR_CONFIG_STORAGE_KEY]: config }, () => {
          console.log(`[SyncSellr BG] Selector config cached: v${config.version}`);
          resolve();
        });
      });
    }
  } catch (err: any) {
    console.warn('[SyncSellr BG] Selector config refresh failed:', err?.message || err);
  }
}
...
async function runHealthChecks(): Promise<void> {
  console.log('[SyncSellr BG] Running selector health checks...');
  try {
    const cached = await new Promise<any>((resolve) => {
      chrome.storage.local.get([SELECTOR_CONFIG_STORAGE_KEY], (result) => {
        resolve(result[SELECTOR_CONFIG_STORAGE_KEY] || null);
    ...
    if (!cached?.platforms) {
      console.warn('[SyncSellr BG] No selector config cached, skipping health check');
      return;
    }
    ...
    for (const [platform, url] of Object.entries(PLATFORM_URLS)) {
      ...
      try {
        ...
        if (execResults?.[0]?.result) {
          ...
          console.log(`[SyncSellr BG] Health check ${platform}: ${execResults[0].result.overallStatus}`);
        }

        // Close the tab
        await chrome.tabs.remove(tabId);
      } catch (err: any) {
        console.warn(`[SyncSellr BG] Health check failed for ${platform}:`, err?.message || err);
        results[platform] = {
          platform,
    ...
    await new Promise<void>((resolve) => {
      chrome.storage.local.set({ [SELECTOR_HEALTH_STORAGE_KEY]: results }, () => {
        console.log('[SyncSellr BG] Health check results stored');
        resolve();
      });
    });
  } catch (err: any) {
    console.warn('[SyncSellr BG] Health checks failed:', err?.message || err);
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  handleMessage(message, sender)
    .then(sendResponse)
    .catch((error) => {
      console.error('[SyncSellr BG] Unhandled error in message handler:', error);
...
async function crossList(listing: any, marketplace: string) {
  console.log('[SyncSellr BG] crossList called for', marketplace, 'listing:', listing.id);
  ...
  if (token) {
    try {
      ...
      console.log('[SyncSellr BG] Full listing fetched:', fullListing.title, 'images:', fullListing.images?.length);
      listing = { ...listing, ...fullListing };
    } catch (err) {
      console.warn('[SyncSellr BG] Failed to fetch full listing, using popup data:', err);
    }
  } else {
    console.warn('[SyncSellr BG] No auth token found');
  }
  ...
  if (listing.images?.length > 0) {
    console.log('[SyncSellr BG] Pre-fetching', listing.images.length, 'images as data URLs...');
    ...
    for (const img of listing.images) {
      ...
      try {
        ...
        console.log('[SyncSellr BG] Image fetched:', imgUrl.substring(imgUrl.lastIndexOf('/') + 1));
      } catch (err: any) {
        console.warn('[SyncSellr BG] Failed to fetch image:', imgUrl, err.message);
      }
    }
    listing.images = resolvedImages;
    console.log('[SyncSellr BG] Pre-fetched', resolvedImages.length, 'images');
  }
  ...
  if (marketplace === 'etsy') {
    console.log('[SyncSellr BG] Etsy uses API integration, not browser automation');
  ...
  console.log('[SyncSellr BG] Opening tab:', url);
  const tab = await chrome.tabs.create({ url });
  console.log('[SyncSellr BG] Tab created with id:', tab.id);

  if (tab.id === undefined) {
    console.error('[SyncSellr BG] Tab created but has no ID');
  ...
  return new Promise((resolve) => {
    ...
    const timeoutId = setTimeout(() => {
      if (!resolved) {
        ...
        console.error('[SyncSellr BG] Tab load timed out after', TAB_LOAD_TIMEOUT_MS, 'ms');
    ...
    function listener(updatedTabId: number, changeInfo: chrome.tabs.TabChangeInfo) {
      if (updatedTabId === tabId && changeInfo.status === 'complete') {
        ...
        console.log('[SyncSellr BG] Tab loaded, waiting 3s before sending message...');

        // Longer delay for heavy JS to render the form
        setTimeout(() => {
          if (resolved) return;
          resolved = true;
          console.log('[SyncSellr BG] Sending CREATE_LISTING to tab', updatedTabId, 'data:', {
            title: listing.title,
            price: listing.price,
          ...
          chrome.tabs.sendMessage(updatedTabId, {
            ...
            console.log('[SyncSellr BG] Message sent successfully');
          }).catch((err: any) => {
            console.error('[SyncSellr BG] Failed to send message to tab:', err?.message || err);
          });
...
async function startListingFlow(
  ...
  try {
    ...
    console.error('[SyncSellr BG] startListingFlow failed:', err?.message || err);
